...
/datum/component/eora_bond/proc/on_heal(datum/source, healing_on_tick, healing_datum)
	if( !isliving(parent) || source != parent || istype(healing_datum, /datum/status_effect/buff/healing/eora) || HAS_TRAIT(parent, TRAIT_PSYDONITE) || HAS_TRAIT(parent, TRAIT_NOFAITHHEAL))
		return
...
/obj/item/fruit_of_eora/proc/update_pom()
	switch(fruit_tier)
		...
		if(2)
			...
			desc = "A blemished pomegranate, it's blue like blortz."
