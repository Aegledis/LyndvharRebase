...
/obj/item/paper/scroll/grudge
	...
	desc = "A copy you've taken with you. Unfortunately the dampness of Lyndhardtia made it unreadable. You can still add new entries, however. It looks bulky enough to act as a mild blunt weapon."
...
/obj/item/paper/scroll/cargo/proc/rebuild_info()
	...
	if(signedname)
		info += "SIGNED,<br/>"
		info += "<font face=\"[FOUNTAIN_PEN_FONT]\" color=#27293f>[signedname] the [signedjob] of Lyndvhar</font>"
...
/obj/item/paper/scroll/sell_price_changes/New(loc, list/prices, faction_name)
	...
	if(!faction)
		faction = pick("Weocilyn", "Greencrest", "Grenzelhoft", "Aevislan")		//add more as time goes, idk
